...
**Note:** <span style="color: cornflowerblue"> `where()` </span> is an example of a `tidyselect` helper function. It selects the variables for which some predicate function , such as <span style="color: cornflowerblue"> `is.numeric()` </span>, returns `TRUE`. 
...
.panel[.panel-name[Output]
```{r ref.label="across-example", echo=FALSE, message=FALSE}
```
]
]
