...
`summarize_if()` is another example of a **scoped verb**. It is a special variant of `summarize()` that applies a summary function to a set of columns that all satisfy some logical criteria. That logical criteria is specified by a **predicate function**, e.g. `is.numeric()`. 
...
`...` = additional arguments for the function(s) specified in `.fns`

`.names` = a `glue` specification that describes how you want to name the output columns of your transformation
---
# `dplyr::across()`
...
penguins %>% 
  group_by(species) %>% 
  summarize(across(where(is.numeric), mean, na.rm = TRUE, .names = "{col}_mean"))
```
