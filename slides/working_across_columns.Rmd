...
- **Understand** how to apply `dplyr` functions, e.g. `mutate()`, `summarize()`, to single columns

--

- **Understand** how to apply `dplyr` functions to multiple columns using scoped verbs, e.g. `summarize_if()`, `summarize_at()`
...
**Note:** `where()` is an example of a `tidyselect` helper function. It selects the variables for which some predicate function , such as `is.numeric()`, returns `TRUE`. 
