...
- Frequently uses common `dplyr` verbs for simple data transformations, e.g. creating new variables, summarizing single variables
]
...
Calculate the mean value of **all bill dimensions** (i.e. `bill_length`, `bill_depth`) for each species.

`r emo::ji("no_entry_sign")` Avoid copying and pasting or repeating `mean()` more than once in your solution, e.g. 
...
Calculate the mean value of **all numeric** variables for each species.
